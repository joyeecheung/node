...
class ModuleLoader {
  ...
  getModuleJobSync(specifier, parentURL, importAttributes, isKnownESM) {
    const resolveResult = this.resolveSync(specifier, parentURL, importAttributes);
    return this.getJobFromResolveResult(resolveResult, parentURL, importAttributes, true, isKnownESM);
  }

  getJobFromResolveResult(resolveResult, parentURL, importAttributes, sync, isKnownESM) {
    ...
    if (job === undefined) {
      job = this.#createModuleJob(url, resolvedImportAttributes, parentURL, format, sync, isKnownESM);
    }
   ...
   * @returns {Promise<ModuleJob>} The (possibly pending) module job
   */
  #createModuleJob(url, importAttributes, parentURL, format, sync, isKnownESM) {
    ...
    const moduleProvider = sync ?
      (url, isMain) => callTranslator(this.loadSync(url, context, isKnownESM), isMain) :
