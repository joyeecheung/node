...
class MockModuleContext {
  ...
  constructor({
    ...
    require('internal/test_runner/mock/loader').register({
      __proto__: null,
      baseURL,
      cache,
  ...
  restore() {
    ...
    require('internal/test_runner/mock/loader').unregister({
      __proto__: null,
...
class MockTracker {
  ...
  module(specifier, options = kEmptyObject) {
    ...
    const { format, url } = sharedState.moduleLoader.resolveSync(
      mockSpecifier, caller, { __proto__: null },
    );
...
function setupSharedModuleState() {
  if (sharedModuleState === undefined) {
    ...
    const { resolve, load } = require('internal/test_runner/mock/loader');
