...
const { initializeAsyncLoaderHooksForLoaderWorker } = require('internal/modules/esm/utils');
...
async function customizedModuleWorker(lock, syncCommPort, errorHandler) {
  ...
  try {
    hooks = await initializeAsyncLoaderHooksForLoaderWorker();
  } catch (exception) {
  ...
  async function handleMessage({ method, args, port }) {
    ...
    assert(typeof hooks[method] === 'function', `${method} is not implemented in the loader worker`);
