...
class ProbeInspectorSession {
  constructor(options) {
    ...
    this.results = [];
    this.timeout = null;
    /** @type {{ index: number, location: Location } | null} */
    // non-null while a probe's `Debugger.evaluateOnCallFrame`
    ...
    // when a non-evaluate CDP call rejects.
  ...
  // Probing process should exit with 0, recorded hits are trustworthy.
  finishWithTrustedResult(terminal) {
    this.finish(kNoFailure, terminal);
  }

  // Probing process should exit with 1, recorded hits are best-effort.
  ...
  makeTargetExitTerminal(exitCode, signal) {
    const pending = this.getPendingProbeIndices();
    const how = signal !== null ? `with signal ${signal}` : `with code ${exitCode}`;
    const what = pending.length === 0 ?
      'completion' : `probes: ${formatPendingProbeLocations(this.probes, pending)}`;
    const error = {
      __proto__: null,
      code: 'probe_target_exit',
      message: `Target exited ${how} before ${what}`,
    };
  ...
  onChildExit(code, signal) {
    ...
    if (!this.started || code !== 0 || signal !== null) {
      this.finishWithTrustedResult(this.makeTargetExitTerminal(code, signal));
      return;
    }
    const pending = this.getPendingProbeIndices();
  ...
  onClientClose() {
    ...
    if (this.inFlightCdpMethod !== null) {
      if (this.inFlightProbe !== null || this.lastProbeIndex !== null) {
        this.markInspectorFailure({
  ...
  // Idempotent chokepoint for the probe_failure terminal.
  markInspectorFailure({ reason, advice, cdpError }) {
    if (this.finished) { return; }
    const child = this.child;
    const exitedAbnormally = child !== null &&
      (child.signalCode !== null || (child.exitCode !== null && child.exitCode !== 0));
    // Yield to `probe_target_exit` only when there is no probe attribution
    // to give. An in-flight probe always classifies as `probe_failure`.
    if (exitedAbnormally && this.inFlightProbe === null) {
      this.finishWithTrustedResult(this.makeTargetExitTerminal(child.exitCode, child.signalCode));
      return;
    }

    const failedCdpMethod = this.inFlightCdpMethod;
    ...
    if (failedCdpMethod !== null) {
      const details = { __proto__: null, lastCdpMethod: failedCdpMethod };
    ...
    this.timeout = setTimeout(() => {
      ...
      if (this.inFlightCdpMethod !== null) {
        const hasProbeAttribution =
          this.inFlightProbe !== null || this.lastProbeIndex !== null;
        this.markInspectorFailure({
