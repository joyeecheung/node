...
class HookedModuleLoader extends DefaultModuleLoader {
  constructor(hooks) {
    super();
    this[kHooks] = hooks;
    this.loaderType = 'internal';
  }
  async #getModuleJob(specifier, parentURL, importAssertions) {
    const resolveResult = await this[kHooks].resolve(specifier, parentURL, importAssertions);
    return this.getJobFromResolveResult(resolveResult, parentURL, importAssertions);
  }
  getModuleJob(specifier, parentURL, importAssertions) {
    const jobPromise = this.#getModuleJob(specifier, parentURL, importAssertions);
  ...
  load(url, context) { return this[kHooks].load(url, context); }
}
...
function createModuleLoader(useCustomLoadersIfPresent = true) {
  if (isLoaderWorker()) {
    ...
    const hookedModuleLoader = new HookedModuleLoader(new Hooks());
    return hookedModuleLoader;
  }
