...
// getOptionValue() can only be called at runtime, in the accessors,
// but it can be created earlier here as long as it's not invoked
// during the initial bootstrap (if it's invoked by accident
// an assertion error would be thrown).
...
function addReadOnlyProcessAlias(name, option, enumerable = true) {
  ObjectDefineProperty(process, name, {
    ...
    enumerable,
    get() {
      // Legacy behavior: if it's falsy, set it to undefined
      // but writable, otherwise it's non-writable.
      const value = getOptionValue(option) || undefined;
      ObjectDefineProperty(process, name, {
