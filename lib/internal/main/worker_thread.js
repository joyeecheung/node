...
port.on('message', (message) => {
  if (message.type === LOAD_SCRIPT) {
    ...
    // The loader thread spawned for handling asynchronous loader hooks should not
    // further spawn other loader threads or there will be an infinite recursion.
    const shouldSpawnLoaderWorker = !isLoaderWorker;
    // The loader thread for async loader hooks will preload user modules itself in
    // initializeAsyncLoaderHooksForLoaderWorker().
    const shouldPreloadModules = !isLoaderWorker;
