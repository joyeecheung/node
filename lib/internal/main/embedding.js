...
function embedderRunESM(content) {
  const filename = process.execPath;
  const url = pathToFileURL(filename);
  const { compileSourceTextModule } = require('internal/modules/esm/utils');
  // TODO(joyeecheung): This needs special handling for dynamic import()
  // to support loading modules from the assets bundled in the executable.
  // TODO(joyeecheung): support code cache loading.
  const wrap = compileSourceTextModule(url, content);
  // Cache the source map for the module if present.
  if (wrap.sourceMapURL) {
    maybeCacheSourceMap(url, content, wrap, false, undefined, wrap.sourceMapURL);
  }
  ...
  for (let i = 0; i < moduleRequests.length; ++i) {
    ...
    const module = loadBuiltinModule(StringPrototypeSlice(specifier, 5));
    if (!module) {
      throw new ERR_UNKNOWN_BUILTIN_MODULE(specifier);
    }
    modules.push(module);
    wrap.link(specifiers, modules);
    wrap.instantiateSync();
    wrap.evaluateSync();
  }
}

return [process, embedderRequire, embedderRunCjs, embedderRunESM];
