...
function setupFetch() {
  ...
  const lazyWasmStreamingCallback = getLazy(() => require('internal/wasm_web_api').wasmStreamingCallback);
  internalBinding('wasm_web_api').setImplementation((streamState, source) => {
    lazyWasmStreamingCallback()(streamState, source);
  });
}
