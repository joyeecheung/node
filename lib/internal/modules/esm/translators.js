...
function makeRequireForESM(module, url) {
  const cascadedLoader = require('internal/modules/esm/loader').getOrInitializeCascadedLoader();
 ...
 */
function loadCJSWithESMLoader(module, source, url, filename, isMain) {
  ...
  const requireFn = getOptionValue('--experimental-require-module') ?
    makeRequireForCJS(module, url) :
    makeRequireForESM(module, url);
...
function loadCJSWithCJSLoader(module, source, url, filename, isMain) {
  assert(module === CJSModule._cache[filename]);
  // Source is indirectly passed from cjsParseCache.
 ...
 * @param {boolean} useCJSLoader - Use the ESM loader or the CJS loader.
 * @returns {ModuleWrap} The ModuleWrap object for the CommonJS module.
 */
function createCJSModuleWrap(url, source, isMain, useCJSLoader) {
  ...
  return new ModuleWrap(url, undefined, namesWithDefault, function() {
    ...
    if (!module.loaded) {
      if (useCJSLoader) {
        loadCJSWithCJSLoader(module, source, url, filename, isMain);
      } else {
        loadCJSWithESMLoader(module, source, url, filename, isMain);
      }
    }
...
translators.set('require-commonjs', (url, source, isMain) => {
  ...
  const useCJSLoader = getOptionValue('--experimental-require-module');
  return createCJSModuleWrap(url, source, isMain, useCJSLoader);
});
...
translators.set('commonjs', /* async */function commonjsStrategy(url, source,
  ...
  const useCJSLoader = source == null || getOptionValue('--experimental-require-module');
