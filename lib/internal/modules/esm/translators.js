...
translators.set('module', function moduleStrategy(url, source, isMain) {
  ...
  if (filename) {
    const cjsModule = cjsEmplaceModuleCacheEntry(filename);
...
function loadCJSModule(module, source, url, filename, isMain) {
  ...
  const requireFn = function require(specifier) {
    ...
    // FIXME(node:59666) Currently, the ESM loader re-invents require() here for imported CJS and this requires a separate
    // cache to be populated and invokes the hooks twice. This is not ideal.
    ...
    assert(mod, `Imported CJS module ${url} failed to load module ${job.url} in cache for require()`);
    return mod.exports;
  };
