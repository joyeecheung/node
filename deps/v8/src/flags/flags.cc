...
struct Flag {
  ...
  bool ImpliedBy(const void* ptr) const {
    const Flag* current = this;
    while (current->implied_by_ptr_ != nullptr) {
      if (current->implied_by_ptr_->PointsTo(ptr)) return true;
      current = static_cast<const Flag*>(current->implied_by_ptr_);
    }
    return false;
  }
#endif
  ...
  bool CheckFlagChange(SetBy new_set_by, bool change_flag,
    ...
    if (IsAnyImplication(new_set_by)) {
      ...
      DCHECK_NOT_NULL(implied_by_ptr_);
#endif
    }
    return change_flag;
  }
