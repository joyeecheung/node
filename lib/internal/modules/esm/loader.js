...
class ModuleLoader {
   ...
   */
  import(specifier, parentURL, importAttributes, phase = kEvaluationPhase, isEntryPoint = false) {
    const request = { specifier, phase, attributes: importAttributes, __proto__: null };

    const promise = this.doImport(parentURL, request, isEntryPoint);
    if (onImport.hasSubscribers) {
      const context = { __proto__: null, parentURL, url: specifier };
      return onImport.tracePromise(promise, context);
    }
    return promise;
  }

  async doImport(parentURL, request, isEntryPoint) {
    let maybePromise;
    try {
      maybePromise = this.getOrCreateModuleJob(parentURL, request);
    } catch (e) {
    ...
    const moduleJob = isPromise(maybePromise) ? await maybePromise : maybePromise;

    if (request.phase === kSourcePhase) {
