...
class ProbeInspectorSession {
  ...
  // Idempotent chokepoint for the probe_failure terminal. Yields
  ...
  markInspectorFailure({ prefix, advice, cdpMethod, cdpError } = {}) {
    ...
    const message = `${prefix}${suffix}. ${advice}`;
  ...
  buildReport(state) {
    ...
    if (state?.event === 'error' && state.code === 'probe_target_exit') {
      const status = pending.length === 0 ? 'completion' : `probes: ${formatPendingProbeLocations(this.probes, pending)}`;
      const message = `Target exited before ${status}`;
      const error = { __proto__: null, code: 'probe_target_exit', message };
    ...
    if (state?.event === 'error' && state.code === 'probe_failure') {
      const error = {
        __proto__: null,
        code: 'probe_failure',
