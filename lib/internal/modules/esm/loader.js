...
class ModuleLoader {
  ...
  async eval(source, url) {
    ...
    const job = new ModuleJob(
      this, url, undefined, ReflectApply(evalInstance, this, [url]), false, false);
  ...
  importSyncForRequire(url, source, isMain) {
    ...
    job = new ModuleJob(this, url, kEmptyObject, wrap, isMain, inspectBrk, true);
  ...
  #createModuleJob(url, importAttributes, parentURL, format, sync, requireESMHint) {
    ...
    const job = new ModuleJob(
      ...
      url,
      importAttributes,
      ReflectApply(moduleProvider, this, [url, isMain]),
      isMain,
      inspectBrk,
      sync,
    );
