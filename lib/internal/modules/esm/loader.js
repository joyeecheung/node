...
class ModuleLoader {
  ...
  #getOrCreateModuleJobAfterResolve(parentURL, resolveResult, request, requestType) {
    ...
    const importAttributes = resolveResult.importAttributes ?? request.importAttributes;
    ...
    if (job !== undefined) {
      ...
      job.ensurePhase(phase, requestType);
      return job;
    }
  ...
  resolve(parentURL, request) {
    if (this.isForAsyncLoaderHookWorker) {
      ...
      const importAttributes = request.importAttributes ?? kEmptyObject;
