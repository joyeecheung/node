...
const { kModuleWrapForCJS, Module: CJSModule, kIsExecuting } = require('internal/modules/cjs/loader');
...
const { ModuleWrap, kEvaluating } = internalBinding('module_wrap');
const {
  urlToFilename,
...
class ModuleLoader {
  ...
  importSyncForRequire(mod, filename, source, isMain, parent) {
    ...
    //    is already completed and we can check the synchronicity of the graph.
    //    Just check whether the graph is synchronous and if it is, return the
    //    live binding of the module namespace. If the evaluation is not yet
    //    finished (even when the graph is synchronous), the CJS side will only get
    //    the populated namespace when the evaluation completes.
    //    TODO(joyeecheung): ensure that synchronous graphs are always evaluated
    //    synchronously so that the namespace of a synchronous subgraph is always
    //    populated at this point.
    // 2. If it was loaded by `import` before, the instantiation may not have
    //    been completed yet. If it is already instantiated, check whether it's
    //    synchronous and return the module namespace if it is. Otherwise, an
    //    error is thrown.
    //    TODO(joyeecheung): make the instantiation phase fully synchronous, so
    //    that 2 behaves the same as 1.
    if (job !== undefined) {
      mod[kModuleWrapForCJS] = job.module;
      return job.module.getNamespaceSync(parent?.filename);
    }
  ...
  getModuleWrapForRequire(specifier, parentURL, importAttributes) {
    ...
    if (finalFormat === 'commonjs-sync') {
      // Check if the ESM initiating import CJS is being required by the same CJS module.
      const filename = urlToFilename(url);
      const mod = CJSModule._cache[filename];
      if (mod && mod[kIsExecuting]) {
        const parentFilename = urlToFilename(parentURL);
