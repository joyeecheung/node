...
class ModuleLoader {
  ...
  getModuleForImportInRequire(specifier, parentURL, importAttributes, requireESMHint) {
    ...
    if (resolveResult == null) {
      resolveResult = this.defaultResolve(specifier, parentURL, importAttributes, requireESMHint);
    ...
    if (job !== undefined) {
      // TODO(joyeecheung): throw if it's async.
      return job.module;
    }
    ...
    // TODO(joyeecheung): override translator for commonjs.
    ...
    const wrap = FunctionPrototypeCall(translator, this, responseURL, source, isMain, parentURL);
    assert(wrap instanceof ModuleWrap);
  ...
  importSyncForRequire(url, source, isMain) {
    ...
    if (job !== undefined) {
      // The job has been previously created.
      const { ModuleJobSync } = require('internal/modules/esm/module_job');
      if (job instanceof ModuleJobSync) {
        return job.module.getNamespace();
      }
      assert(job.instantiated !== undefined);
    ...
    // TODO(joyeecheung): return the namespace directly.
