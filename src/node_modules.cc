...
void CompileCacheFor(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
  Environment* env = Environment::GetCurrent(context);
  CHECK(args[0]->IsArray());

  CHECK(!env->use_compile_cache());

  Local<Array> input = args[0].As<Array>();
  std::vector<Global<Value>> array;
  CHECK(FromV8Array(context, input, &array).IsJust());

  std::vector<node::CompileCacheEntry *> cache_entries;
  for (size_t i = 0; i < array.size(); i++) {
    HandleScope handle_scope(isolate);
    Local<Value> item = array[i].Get(isolate);
    DCHECK(item->IsObject());
    Local<Object> obj = item.As<Object>();
    Local<Value> filename_val = obj->Get(context, FIXED_ONE_BYTE_STRING(isolate, "filename")).ToLocalChecked();
    Local<Value> source_val = obj->Get(context, FIXED_ONE_BYTE_STRING(isolate, "source")).ToLocalChecked();
    Local<Value> type_val = obj->Get(context, FIXED_ONE_BYTE_STRING(isolate, "type")).ToLocalChecked();
    CHECK(filename_val->IsString());
    CHECK(source_val->IsString());
    CHECK(type_val->IsUint32());

    Local<String> filename = filename_val.As<String>();
    Local<String> source = source_val.As<String>();
    CachedCodeType type = static_cast<CachedCodeType>(type_val.As<v8::Uint32>()->Value());
    CHECK(type == CachedCodeType::kCommonJS || type == CachedCodeType::kESM);

    auto* cache_entry =
        env->compile_cache_handler()->GetOrInsert(source, filename, type);
    CHECK_NOT_NULL(cache_entry);
    ...
    switch (type) {
      case CachedCodeType::kCommonJS: {
        Debug(env,
              DebugCategory::COMPILE_CACHE,
              "[compile cache] caching CommonJS module %s %s\n",
      ...
      case CachedCodeType::kESM: {
        Debug(env,
              DebugCategory::COMPILE_CACHE,
              "[compile cache] caching ESM module %s %s\n",
              cache_entry->type_name(),
              cache_entry->source_filename);
        break;
      }
