...
// This file is the main script used when building a user snapshot. We do not use it to build the
// built-in snapshot (for that, we only bake the environment by running internal/bootstrap/* scripts,
// the main script is only run during run time).

// The snapshot building process starts from LoadEnvironment() either called internally or called as
// part of the public embedder API on an Environment baked with CreateForSnapshotting().
// LoadEnvironment() -> StartExecution() -> internal/main/mksnapshot 
const {
  Error,
...
const { runEntryPoint } = internalBinding('embedding');
...
function stashErrorStackTraceLimit() {
...
stashErrorStackTraceLimit();
prepareMainThreadExecution(false, false);
if (isExperimentalSeaWarningNeeded()) {
  emitExperimentalWarning('Single executable application');
}

runEntryPoint(process, requireForUserSnapshot, );
