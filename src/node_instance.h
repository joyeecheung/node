#ifndef SRC_NODE_INSTANCE_H_
#define SRC_NODE_INSTANCE_H_
...
class NodeInstance {
 public:
  // To create an instance that does not own the isolate,
  ...
  //   isolate->Initialize(...);
  //   isolate->Enter();
  //   std::unique_ptr<NodeInstance> main_instance =
  //       NodeInstance::Create(isolate, loop, args, exec_args);
  //
  ...
  //
  // After calling Dispose() the instance is no longer accessible.
 ...
 private:
  NodeInstance(v8::Isolate* isolate,
               uv_loop_t* event_loop,
               MultiIsolatePlatform* platform,
               const std::vector<std::string>& args,
               const std::vector<std::string>& exec_args);
