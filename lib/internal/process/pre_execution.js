...
function setupHttpProxy() {
  const useEnvProxy = getOptionValue('--use-env-proxy');
  const fetchEnvProxy = getOptionValue('--fetch-env-proxy');

  if (!useEnvProxy && !fetchEnvProxy) {
    return;
  }
  ...
  if (fetchEnvProxy) {
    // Set the global dispatcher for fetch(). This is controlled by
    // --fetch-env-proxy (or --use-env-proxy which implies --fetch-env-proxy).
  ...
  // actually created, in lib/_http_agent.js and lib/https.js. That is
  // controlled by --use-env-proxy only.
}
