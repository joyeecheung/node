...
let translators, _cjsEmplaceModuleCacheEntryForURL;
function lazyLoadTranslators() {
  if (!translators) {
    ({
      translators,
      cjsEmplaceModuleCacheEntryForURL: _cjsEmplaceModuleCacheEntryForURL,
    } = require('internal/modules/esm/translators'));
  }
  return { translators, _cjsEmplaceModuleCacheEntryForURL };
}

function cjsEmplaceModuleCacheEntryForURL(url, parent) {
  return lazyLoadTranslators()._cjsEmplaceModuleCacheEntryForURL(url, parent);
}
...
class ModuleLoader {
  ...
  #createModuleJob(url, importAttributes, phase, parentURL, format, isForRequireInImportedCJS) {
    ...
    // Populate the separate CJS cache for the reinvented require() in imported CJS.
    if (format === 'module') {
      const parentFilename = urlToFilename(parentURL);
      const parent = parentFilename ? CJSModule._cache[parentFilename] : undefined;
      cjsEmplaceModuleCacheEntryForURL(url, parent);
    }
