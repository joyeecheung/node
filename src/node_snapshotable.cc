...
std::vector<char> SnapshotData::ToBlob() const {
  ...
  SnapshotSerializer w(IsRODataCompressed(metadata.flags));
  ...
  // TODO(joyeecheung): if compress_rodata_ is true, compress it first.
...
bool SnapshotData::MaybeDecompress() {
  // TODO(joyeecheung): decompress v8_snapshot_blob_data and
  // code_cache, remember to change ownership so that it's owned
  // by the SnapshotData as decompression results in allocation.
}
...
bool SnapshotData::FromBlob(SnapshotData* out, std::string_view in) {
  SnapshotDeserializer r(in, IsRODataCompressed(out->metadata.flags));
  r.Debug("SnapshotData::FromBlob()\n");

  DCHECK_EQ(out->data_ownership, SnapshotData::DataOwnership::kOwned);

  // TODO(joyeecheung): if compress_rodata_ is true,de compress it first.
  // Metadata
...
void WriteByteVectorLiteral(std::ostream* ss,
  ...
  // TODO(joyeecheung): compress vec first when compress_vector is true.
...
namespace node {
  ...
  WriteByteVectorLiteral(&ss,
                         ...
                         use_array_literals,
                         IsRODataCompressed(data->metadata.flags));
...
const SnapshotData* SnapshotBuilder::GetEmbeddedSnapshotData() {
  CHECK(snapshot_data.MaybeDecompress());
...
bool IsRODataCompressed(const SnapshotFlags& flags) {
  return static_cast<bool>(flags & SnapshotFlags::kCompressROData);
}

bool IsRODataCompressed(const SnapshotConfig& config) {
  return IsRODataCompressed(config.flags);
}
