...
class ModuleJobBase {
   ...
   * @param {Function} iterator Runs on the request.
   * @returns {Promise<ModuleJob[]>} Dependency module jobs.
   */
  #forEachModuleRequest(iterator) {
    ...
    for (
      ...
      const { specifier, phase, attributes } = request;
      ...
      iterator(idx, eidx, request);
    }
  }

  /**
   * Synchronously link the module and its dependencies.
   * @param {kImportInImportedESM | kImportInRequiredESM | kRequireInImportedCJS} requestType
   * @returns {Array<ModuleJob>}
   */
  syncLink(requestType) {
    ...
    try {
      this.#forEachModuleRequest((idx, eidex, request) => {
        const job = this.#loader.getOrCreateModuleJob(this.url, request, requestType);
        debug(`ModuleJobBase.syncLink() ${this.url} -> ${specifier}`, job);
...
class ModuleJob extends ModuleJobBase {
   ...
   * @param {kImportInRequiredESM | kRequireInImportedCJS | kImportInImportedESM} requestType
   */
  ...
  async #asyncLink(requestType) {
    ...
    this.#forEachModuleRequest((idx, eidx, request) => {
      const dependencyJobPromise = this.#loader.getModuleJobForImport(this.url, request);
      const modulePromise = PromisePrototypeThen(dependencyJobPromise, (job) => {
        debug(`ModuleJob.link() ${this.url} -> ${specifier}`, job);
        if (phase === kEvaluationPhase) {
          evaluationDepJobs[eidx] = job;
        }
