...
const {
  ...
  encodings: { INVALID_ENCODING, HEX, BUFFER },
...
Hash.prototype._flush = function _flush(callback) {
  this.push(this[kHandle].digest(BUFFER));
  callback();
};
...
Hash.prototype.digest = function digest(outputEncoding) {
  ...
  // Coerce to BUFFER for non-string values for backward compatibility.
  let normalized = BUFFER;
  if (typeof outputEncoding === 'string') {
    normalized = normalizeEncodingStrict(outputEncoding);
    if (normalized === INVALID_ENCODING) {
      normalized = BUFFER;
    }
  }
  const ret = this[kHandle].digest(normalized);
