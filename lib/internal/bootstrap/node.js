...
/* global globalProxy, process, bootstrappers, loaderExports */
/* global triggerFatalException */
...
function startup() {
  ...
  if (process.platform === 'win32') {
    ...
    for (var n = 0; n < names.length; n++) {
      Object.defineProperty(globalProxy, `COUNTER_${names[n]}`, {
        configurable: true,
        enumerable: false,
...
function executeUserCode() {
  ...
  if (process._eval != null && !process._forceRepl) {
    ...
    addBuiltinLibsToObject(globalProxy);
...
function setupGlobalVariables() {
  Object.defineProperty(globalProxy, Symbol.toStringTag, {
    value: 'global',
    writable: false,
    enumerable: false,
    configurable: true
  });
  globalProxy.process = process;
  ...
  Object.defineProperties(globalProxy, {
    GLOBAL: {
      configurable: true,
  ...
  globalProxy.Buffer = NativeModule.require('buffer').Buffer;
  process.domain = null;
  process._exiting = false;

  // This allows users to access the global proxy object via `global`.
  // The builtin modules can access this either through `globalProxy`
  // in the parameters passed through
  // `ScriptCompiler::CompileFunctionInContext()`, or by directly referencing
  // `global`, though the latter would fail the linter.
  // Users, of course, can only access this through `global`.
  globalProxy.global = globalProxy;
}

function setupGlobalTimeouts() {
  const timers = NativeModule.require('timers');
  globalProxy.clearImmediate = timers.clearImmediate;
  globalProxy.clearInterval = timers.clearInterval;
  globalProxy.clearTimeout = timers.clearTimeout;
  globalProxy.setImmediate = timers.setImmediate;
  globalProxy.setInterval = timers.setInterval;
  globalProxy.setTimeout = timers.setTimeout;
}

function setupGlobalConsole() {
  const consoleFromVM = globalProxy.console;
  const consoleFromNode =
    NativeModule.require('internal/console/global');
  // Override global console from the one provided by the VM
  // to the one implemented by Node.js
  Object.defineProperty(globalProxy, 'console', {
    configurable: true,
    enumerable: false,
...
function setupQueueMicrotask() {
  Object.defineProperty(globalProxy, 'queueMicrotask', {
    get: () => {
