...
/**
 * The type names here follow the same rules as the documentation:
 * - Primitives types are in lower-case, e.g. 'number', 'string', 'boolean'
 * - null is special-cased to 'null'
 * - Non-primitive types are capitalized. If there is a constructor or a tag,
 *   use the same type names as displayed by util.inspect().
 */
function getTypeName(value) {
  ...
  const { constructor, tag } = lazyInternalUtil().getIdentificationOf(value);
...
function invalidArgType(name, expected, actual) {
  ...
  if (arguments.length >= 3) {
    msg += `. Received type ${getTypeName(actual)}`;
  }
  return msg;
}
