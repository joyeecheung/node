...
class ModuleLoader {
   ...
   *                                     point.
   * @param {[{specifier: string, attribtues: Record<string, string>}]} moduleRequests
   * @returns {Promise<[ModuleJob]>} The module wraps.
   */
  async getModuleJobsForImport(parentURL, moduleRequests) {
    const resolvePromises = [];
    const specifiers = [];
    // Do the specifier -> url resolution of all modules in one pass, then
    // do the url -> source loading of all modules in one pass. This allows maximizing
    // parallelism for synchronous hooks.
    for (let i = 0; i < moduleRequests.length; i++) {
      const { specifier, attributes } = moduleRequests[i];
      ArrayPrototypePush(resolvePromises, this.resolve(specifier, parentURL, attributes));
      ArrayPrototypePush(specifiers, specifier);
    }
    const resolvedResults = await SafePromiseAllReturnArrayLike(resolvePromises);

    const modulePromises = [];
    const moduleJobs = [];
    for (let i = 0; i < resolvedResults.length; i++) {
      const resolveResult = resolvedResults[i];
      const job = this.#getJobFromResolveResult(resolveResult, parentURL, importAttributes, false);
      ArrayPrototypePush(modulePromises, job.modulePromise);
      ArrayPrototypePush(moduleJobs, job);
    }
    const modules = await SafePromiseAllReturnArrayLike(modulePromises);
    return { specifiers, moduleJobs, modules };
  }
  ...
  #createModuleJob(url, importAttributes, parentURL, format, isForRequireInImportedCJS) {
    const callTranslator = ({ format: formatFromLoad, responseURL, source }, isMain) => {
      if (isForRequireInImportedCJS &&
          (formatFromLoad === 'module' || formatFromLoad === 'module-typescript')) {
    ...
    const moduleProvider = isForRequireInImportedCJS ?
  ...
  async import(specifier, parentURL, importAttributes, isEntryPoint = false) {
    return onImport.tracePromise(async () => {
      const { moduleJobs } = await this.getModuleJobsForImport(parentURL, [{specifier, attributes: importAttributes}]);
      const { module } = await moduleJobs[0].run(isEntryPoint);
  ...
  resolve(originalSpecifier, parentURL, importAttributes) {
    ...
    return this.#cachedDefaultResolve(originalSpecifier, parentURL, importAttributes);
  }
   ...
   */
  resolveSync(originalSpecifier, parentURL, importAttributes = { __proto__: null }) {
    if (resolveHooks.length) {
      // Has module.registerHooks hooks, use the synchronous variant that can handle both hooks.
      return resolveWithHooks(originalSpecifier, parentURL, importAttributes,
                              this.#resolveWithMaybeOffThreadHooks.bind(this));
    }
  ...
  #loadSync(url, context) {
    let result;
    if (loadHooks.length) {
      result = loadWithHooks(url, context.format, this.#loadWithMaybeOffThreadHooks.bind(this));
    } else {
