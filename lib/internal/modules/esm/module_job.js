...
class ModuleJobSync extends ModuleJobBase {
  ...
  async run() {
    // This can only be hit when a later import hits a previously require()'d module.
    // In this case, the module must've been evaluated already because we always
    // evaluate it right after the module is created.
    assert(this.#evaluated);
    return { __proto__: null, module: this.module };
  }

  runSync() {
    assert(!this.#evaluated);  // This should not be run twice.
    assert(this.module instanceof ModuleWrap);
