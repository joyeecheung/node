...
testChildProcess(
  (port) => [`--inspect=${port}`, '--build-snapshot', entry], 12);
testChildProcess(
  (port) => [`--inspect=${port}`], 1);
...
testOnServerListen((server) => {
  ...
  worker.on('error', common.mustCall(common.expectsError({
    code: 'ERR_WORKER_INIT_FAILED',
    message: 'Worker initialization failure: Failed to create Worker'
  })));
