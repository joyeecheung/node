...
const {
  privateSymbols: {
    module_callbacks_symbol,
  },
} = internalBinding('util');

function setCallbackForWrap(wrap, data) {
  wrap[module_callbacks_symbol] = data;
}
...
function initializeImportMetaObject(wrap, meta) {
  if (wrap[module_callbacks_symbol] !== undefined) {
    const { initializeImportMeta } = wrap[module_callbacks_symbol];
...
async function importModuleDynamicallyCallback(wrap, specifier, assertions) {
  if (wrap[module_callbacks_symbol] !== undefined) {
    const { importModuleDynamically } = wrap[module_callbacks_symbol];
