...
class ModuleLoader {
   ...
   * @param {string} specifier See {@link resolveSync}.
  ...
  resolveSync(specifier, parentURL, importAttributes = { __proto__: null }) {
    if (resolveHooks.length) {
      ...
      return resolveWithHooks(specifier, parentURL, importAttributes,
  ...
  #loadSync(url, context) {
    if (loadHooks.length) {
      // Has module.registerHooks() hooks, chain the asynchronous hooks in the default step.
      return loadWithHooks(url, context.format, this.#loadAndMaybeBlockOnLoaderThread.bind(this));
    }
