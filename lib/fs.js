...
Object.defineProperty(url, 'URLToPath', {
  // eslint-disable-next-line func-name-matching
  value: function URLToPath(path) {
    if (typeof path === 'string' || path instanceof Buffer)
      return path;
    if (!internalURL) {
      internalURL = require('internal/url');
    }
    if (!(path instanceof internalURL.URL))
      return path;
    return internalURL.URLToPath(path);
  }
});
...
fs.access = function(path, mode, callback) {
  ...
  if (handleError((path = url.URLToPath(path)), callback))
    return;
...
fs.accessSync = function(path, mode) {
  handleError((path = url.URLToPath(path)));
  nullCheck(path);
...
fs.exists = function(path, callback) {
  if (handleError((path = url.URLToPath(path)), cb))
    return;
...
fs.rename = function(oldPath, newPath, callback) {
  ...
  if (handleError((oldPath = url.URLToPath(oldPath)), callback))
    return;

  if (handleError((newPath = url.URLToPath(newPath)), callback))
    return;
...
fs.renameSync = function(oldPath, newPath) {
  handleError((oldPath = url.URLToPath(oldPath)));
  handleError((newPath = url.URLToPath(newPath)));
  nullCheck(oldPath);
  nullCheck(newPath);
...
fs.symlink = function(target, path, type_, callback_) {
  ...
  if (handleError((target = url.URLToPath(target)), callback))
    return;
...
fs.symlinkSync = function(target, path, type) {
  ...
  handleError((target = url.URLToPath(target)));
  handleError((path = url.URLToPath(path)));
  nullCheck(target);
  nullCheck(path);
...
fs.link = function(existingPath, newPath, callback) {
  ...
  if (handleError((existingPath = url.URLToPath(existingPath)), callback))
    return;
...
fs.linkSync = function(existingPath, newPath) {
  handleError((existingPath = url.URLToPath(existingPath)));
  handleError((newPath = url.URLToPath(newPath)));
...
FSWatcher.prototype.start = function(filename,
                                     ...
                                     recursive,
                                     encoding) {
  handleError((filename = url.URLToPath(filename)));
  nullCheck(filename);
...
fs.realpath = function realpath(p, options, callback) {
  ...
  options = getOptions(options, {});
  if (handleError((p = url.URLToPath(p)), callback))
    return;
...
function ReadStream(path, options) {
  ...
  handleError((this.path = url.URLToPath(path)));
