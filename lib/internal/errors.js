...
function makeSystemErrorWithCode(code) {
...
function makeNodeErrorWithCode(Base, code) {
...
function getMessage(code, args) {
  ...
  if (typeof msg === 'function') {
    assert(
      ...
      `Code: ${code}; The provided arguments length (${args.length}) ` +
        `does not match the required ones (${msg.length}).`
    );
  ...
  assert(
    ...
    `Code: ${code}; The provided arguments length (${args.length}) ` +
      `does not match the required ones (${expectedLength}).`
  );
  if (args.length === 0)
    return msg;

  args.unshift(msg);
  const formatted = util.format.apply(null, args);
