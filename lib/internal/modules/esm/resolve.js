...
function resolvePackageTarget(packageJSONUrl, target, subpath, packageSubpath,
                              base, pattern, internal, isPathMap, conditions, usedConditions) {
...
function packageExportsResolve(
  packageJSONUrl, packageSubpath, packageConfig, base, conditions, usedConditions) {
 ...
 */
function packageResolve(specifier, base, conditions, usedConditions) {
 ...
 * @param {Set<string>} usedConditions - The conditions that are used.
 * @param {boolean} preserveSymlinks - Whether to preserve symlinks in the resolved URL.
 */
function moduleResolve(specifier, base, conditions, usedConditions, preserveSymlinks) {
  ...
  if (shouldBeTreatedAsRelativeOrAbsolutePath(specifier)) {
    ...
    try {
      ...
      resolved = packageResolve(specifier, base, conditions, usedConditions);
    }
  }
...
function defaultResolve(specifier, context = {}) {
  ...
  let url;
  try {
    // TODO(joyeecheung): if the module condition is used, the format should
    // be set to module.
    url = moduleResolve(
      specifier,
      parentURL,
