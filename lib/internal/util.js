...
const {
  ASCII, UTF8, BASE64, UCS2, LATIN1, HEX, BUFFER, BASE64URL, INVALID_ENCODING,
} = encodings;

function normalizeEncodingBase(enc) {
  // Put common cases at the beginning for fewer comparisons.
  switch (enc) {
    case 'utf8':
    case 'utf-8':
      return UTF8;
...
/**
 * Returns INVALID_ENCODING if it's invalid. Otherise return an integer
 * matching the C++ land node::encoding enum.
 * @param {string} enc User-provided name of the encoding.
 * @returns {number}
 */
function normalizeEncodingStrict(enc) {
  const result = normalizeEncodingBase(enc);
  if (result !== INVALID_ENCODING) {
    return result;
  }
  return normalizeEncodingBase(StringPrototypeToLowerCase(enc));
}

/**
 * Normalize the encoding in a sloppy way for backwards compatibility.
 * null, undefined and '' are coerced to UTF8.
 * @param {string} enc
 * @param {*} defaultValue
 * @returns {integer|*}
 */
function normalizeEncodingSloppy(enc, defaultValue) {
  if (enc == null) {  // Legacy behavior.
    return UTF8;
  }
  const encNum = normalizeEncodingStrict(enc);
  if (encNum !== INVALID_ENCODING) {
    return encNum;
  }
  if (enc === '') {
    return UTF8;  // Legacy behavior.
  }
  return defaultValue;
}


const encodingNames = Object.keys(encodings);
const slowArray = new Array(encodingNames.length);
for (let i = 0; i < encodingNames.length; ++i) {
  const name = encodingNames[i];
  slowArray[encodings[name]] = name === 'UCS2' ? 'utf16le' : StringPrototypeToLowerCase(name);
}
const encodingsArray = Array.from(slowArray);

// Return undefined if there is no match.
// Move the "slow cases" to a separate function to make sure this function gets
// inlined properly. That prioritizes the common case.
function normalizeEncoding(enc) {
  const normalized = normalizeEncodingSloppy(enc, INVALID_ENCODING);
  if (normalized !== INVALID_ENCODING) {
    return encodingsArray[normalized];
  }
  return undefined;
}
...
module.exports = {
  ...
  normalizeEncoding,
  normalizeEncodingStrict,
  normalizeEncodingSloppy,
  once,
  promisify,
