...
bool BuiltinLoader::AddFromDisk(const char* id,
                                const std::string& filename,
                                const UnionBytes& source) {
  BuiltinSourceType type = BuiltinSourceType::kFunction;
  if (filename.ends_with(".mjs")) {
    type = BuiltinSourceType::kSourceTextModule;
  }
...
const BuiltinSource* BuiltinLoader::LoadBuiltinSource(Isolate* isolate,
  ...
  AddExternalizedBuiltin(id, filename.c_str());
#endif  // NODE_BUILTIN_MODULES_PATH
}
...
void BuiltinLoader::AddExternalizedBuiltin(const char* id,
  ...
  AddFromDisk(id, filename, UnionBytes(resource));
}
...
MaybeLocal<Data> BuiltinLoader::LookupAndCompile(
  ...
  // testing anyway.
  Result result = (has_cache && !cached_data_ptr->rejected)
                      ? Result::kWithCache
  ...
  if (has_cache) {
    per_process::Debug(
        ...
        cached_data_ptr->buffer_policy ==
            ...
            ? "BufferNotOwned"
            : "BufferOwned",
        cached_data_ptr->rejected ? "is rejected" : "is accepted");
  }
...
void BuiltinLoader::ImportSync(const FunctionCallbackInfo<Value>& args) {
  Realm* realm = Realm::GetCurrent(args);
  CHECK(args[0]->IsString());
...
void BuiltinLoader::CreatePerIsolateProperties(IsolateData* isolate_data,
  ...
  SetMethod(isolate, target, "importSync", ImportSync);
}
