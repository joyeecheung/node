...
async function initializeHooks() {
  ...
  const hookedModuleLoader = require('internal/process/esm_loader').esmLoader;
  const hooks = hookedModuleLoader[kHooks];
  for (let i = 0; i < customLoaderPaths.length; i++) {
    ...
    const keyedExportsSublist = await hookedModuleLoader.import(
