...
const vm = require('vm');
const { fileURLToPath, pathToFileURL, URLParse } = require('internal/url');
...
function createRepl(inspector) {
  ...
  class SourceSnippet {
    constructor(location, delta, scriptSource, mapped = false) {
      ...
      return ArrayPrototypeJoin(
        ArrayPrototypeMap(
          ...
          (lineText, offset) => {
            ...
            // Breakpoints are tracked at generated locations; match against the
            // original location when listing source-mapped (original) source.
            let isBreakpoint = false;
            ArrayPrototypeForEach(knownBreakpoints, ({ location, originalLocation }) => {
              if (this.mapped) {
                if (originalLocation &&
                originalLocation.url === originalUrl &&
                i === (originalLocation.lineNumber + 1)) {
                  isBreakpoint = true;
                }
              } else if (location &&
  ...
  // Lazily resolve and parse the source map for a script, if it has one.
  // Returns null when the script has no map or the map can't be loaded.
  function getScriptSourceMap(scriptId) {
    const script = knownScripts[scriptId];
    if (!script || !script.sourceMapURL) return null;
    if (script.sourceMap === undefined) {
      script.sourceMap = null;
      try {
        // V8 may report the script url as a bare path (e.g. from a
        // `//# sourceURL=` comment) rather than a file: URL. The source map's
        // `sources` are resolved relative to it, so make it absolute first.
        const baseUrl = URLParse(script.url) ?
          script.url : pathToFileURL(Path.resolve(script.url)).href;
        const payload = dataFromUrl(baseUrl, script.sourceMapURL);
        if (payload) script.sourceMap = new SourceMap(payload);
      } catch {
        // Ignore malformed maps and fall back to the generated source.
      }
    }
    return script.sourceMap;
  }

  // Translate a generated location into the original source it was compiled
  // from. Returns null when there is no usable mapping.
  function mapToOriginalLocation(location) {
    const sourceMap = getScriptSourceMap(location.scriptId);
    if (!sourceMap) return null;
    const entry = sourceMap.findEntry(location.lineNumber,
                                      location.columnNumber ?? 0);
    if (entry.originalSource === undefined ||
        entry.originalLine === undefined) {
      return null;
    }
    const source = getOriginalSource(sourceMap.payload, entry.originalSource);
    if (typeof source !== 'string') return null;
    return {
      url: entry.originalSource,
      lineNumber: entry.originalLine,
      columnNumber: entry.originalColumn ?? 0,
      source,
    };
  }

  // Find a loaded script whose source map provides `sourceUrl` (exact, then
  // suffix match so the user can type a bare filename), and map an original
  // line to a generated CDP location. Returns null when nothing matches.
  function mapToGeneratedLocation(sourceUrl, originalLine) {
    const ids = ObjectKeys(knownScripts);
    for (let i = 0; i < ids.length; i++) {
      const scriptId = ids[i];
      const sourceMap = getScriptSourceMap(scriptId);
      if (!sourceMap) continue;
      const { sources } = sourceMap.payload;
      const matched = ArrayPrototypeIncludes(sources, sourceUrl) ? sourceUrl :
        ArrayPrototypeFind(sources, (s) => StringPrototypeEndsWith(s, sourceUrl));
      if (!matched) continue;
      const { generatedLine, generatedColumn } =
        sourceMap.findEntryReversed(matched, originalLine);
      if (generatedLine === undefined) continue;
      return {
        scriptId,
      ...
      return new SourceSnippet(
        {
          ...
          originalUrl: mapped.url,
        },
        delta, mapped.source, true);
    }
  ...
  function handleBreakpointResolved({ breakpointId, location }) {
    ...
    // Map the resolved (generated) location back to the original source for
    // display. Stored without the source text, which we don't need here.
    const mapped = mapToOriginalLocation(location);
    const originalLocation = mapped && {
      url: mapped.url,
      lineNumber: mapped.lineNumber,
      columnNumber: mapped.columnNumber,
    };
    const isExisting = ArrayPrototypeSome(knownBreakpoints, (bp) => {
      if (bp.breakpointId === breakpointId) {
        ObjectAssign(bp, { location, originalLocation });
        return true;
      }
      return false;
    });
    if (!isExisting) {
      ArrayPrototypePush(knownBreakpoints,
                         { breakpointId, location, originalLocation });
    }
  }
  ...
  function listBreakpoints() {
    ...
    function formatLocation(bp) {
      // Prefer the original source location when the breakpoint is in a
      // source-mapped script.
      if (bp.originalLocation) {
        const { url, lineNumber } = bp.originalLocation;
        return `${getRelativePath(url)}:${lineNumber + 1}`;
      }
      const { location } = bp;
  ...
  function setBreakpoint(script, line, condition, silent) {
    ...
    // When the current script has a source map, the line the user sees (and
    // types) is an original line, so resolve it to a generated location.
    if (line === undefined && typeof script === 'number') {
      const current = getCurrentLocation();
      const original = mapToOriginalLocation(current);
      const location = (original &&
        mapToGeneratedLocation(original.url, script - 1)) || {
    ...
    // setBreakpoint('original-file', line): the name matches an original source
    // in a loaded source map, so resolve to a generated location.
    if (typeof line === 'number') {
      const location = mapToGeneratedLocation(script, line - 1);
      if (location) {
        return PromisePrototypeThen(
          Debugger.setBreakpoint({ location, condition }),
          registerBreakpoint);
  ...
  Debugger.on('paused', ({ callFrames, reason /* , hitBreakpoints */ }) => {
    ...
    const script = knownScripts[scriptId];
    const mapped = mapToOriginalLocation(selectedFrame.location);
    const scriptUrl = mapped ? getRelativePath(mapped.url) :
      (script ? getRelativePath(script.url) : '[unknown]');
    const displayLine = (mapped ? mapped.lineNumber : lineNumber) + 1;

    const header = `${breakType} in ${scriptUrl}:${displayLine}`;
