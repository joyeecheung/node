...
function prepareMainThreadExecution(expandArgv1 = false,
  ...
  if (process.env.NODE_V8_COVERAGE) {
    // This need to be done after setupDebugEnv() because the timers
    // can trigger debug logs.
...
function setupCoverageHooks(dir) {
  ...
  if (process.features.inspector) {
    ...
    } = internalBinding('profiler');
    setCoverageDirectory(coverageDirectory);
    setSourceMapCacheGetter(sourceMapCacheToObject);
    const interval = NumberParseInt(process.env.NODE_V8_COVERAGE_INTERVAL);
    const { TIMEOUT_MAX } = require('internal/timers');

    if (interval && interval >= 1 && interval <= TIMEOUT_MAX) {
      startCoverageInterval(interval);
    } else {
      process.emitWarning('Invalid NODE_V8_COVERAGE_INTERVAL value ' +
                          `${interval} has been ignored`,
                          'Warning');
    }
  } else {
