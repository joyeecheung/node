...
const lazyCompose = getLazy(() => require('internal/streams/compose'));
const lazyAddAbortSignalNoValidate =
  getLazy(() => require('internal/streams/add-abort-signal').addAbortSignalNoValidate);
const {
  isWritable,
  isNodeStream,
...
function compose(stream, options) {
  ...
  const composedStream = lazyCompose()(this, stream);

  if (options?.signal) {
    // Not validating as we already validated before
    lazyAddAbortSignalNoValidate()(
      options.signal,
      composedStream
    );
  }
...
const lazyFrom = getLazy(() => require('internal/streams/readable').from);
...
for (const key of streamReturningOperators) {
  const op = ops[key];
  function fn(...args) {
    if (new.target) {
      throw ERR_ILLEGAL_CONSTRUCTOR();
    }
    return lazyFrom()(ReflectApply(op, this, args));
  }
...
for (const key of promiseReturningOperators) {
  const op = ops[key];
  function fn(...args) {
    if (new.target) {
      throw ERR_ILLEGAL_CONSTRUCTOR();
