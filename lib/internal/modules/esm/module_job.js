...
// This is used to signify that a module job has finished instantiation
// from the instantiation done from its parent. Since it's no more than a
// marker used from the root of the tree and there's not actually any pending
// work, this promise is reused to save overhead.
// Note that this should only be used in the loader thread.
let resolvedPromise;
function getResolvedPromiseSingleton() {
  resolvedPromise ??= PromiseResolve();
...
class ModuleJob extends ModuleJobBase {
  ...
  async #asyncInstantiate() {
    ...
    for (const dependencyJob of jobsInGraph) {
      ...
      dependencyJob.instantiated = getResolvedPromiseSingleton();
    }
  }
