...
let _shouldSpawnLoaderWorker = true;
/**
 * Initializes handling of ES modules.
 * @param {boolean} [shouldSpawnLoaderWorker] Whether the custom loader worker
 *                                            should be spawned later.
 */
function initializeESM(shouldSpawnLoaderWorker = true) {
  _shouldSpawnLoaderWorker = shouldSpawnLoaderWorker;
 ...
 * @returns {boolean}
 */
function shouldSpawnLoaderWorker() {
  return _shouldSpawnLoaderWorker;
}

/**
 * Register asynchronus module loader customization hooks. This should only be run in the loader
 * worker. In a non-loader thread, if any asynchronous loader hook is registered, the
 * loader#customizations are initialized to be a worker proxy that posts the messages to the
 * async loader worker thread. When no asynchronous loader hook is registered, the loader worker
 * is not spawned and module loading is entiredly done in-thread.
 *
 * @returns {Promise<any>}
 */
async function initializeAsyncLoaderHooksForLoaderWorker() {
  const customLoaderURLs = getOptionValue('--experimental-loader');

  const { AsyncLoaderHookOnLoaderWorker } = require('internal/modules/esm/hooks');
  const cascadedLoader = require('internal/modules/esm/loader').getOrInitializeCascadedLoader();

  const hooks = new AsyncLoaderHookOnLoaderWorker();
...
module.exports = {
  registerModule,
  initializeESM,
  initializeAsyncLoaderHooksForLoaderWorker,
  getDefaultConditions,
  getConditionsSet,
  loaderWorkerId: 'internal/modules/esm/worker',
  shouldSpawnLoaderWorker,
  compileSourceTextModule,
  requestTypes: { kImportInImportedESM, kImportInRequiredESM, kRequireInImportedCJS },
};
