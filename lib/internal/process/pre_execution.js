...
function prepareMainThreadExecution(expandArgv1 = false, initializeModules = true) {
  return prepareExecution({
    ...
    initializeModules,
    isMainThread: true,
    shouldSpawnLoaderWorker: initializeModules,
...
function prepareTestRunnerMainExecution(loadUserModules = true) {
  return prepareExecution({
    ...
    isMainThread: true,
    shouldSpawnLoaderWorker: loadUserModules,
...
function prepareShadowRealmExecution() {
  ...
  initializeModuleLoaders({ shouldSpawnLoaderWorker: false, shouldPreloadModules: false });
  const {
    privateSymbols: {
...
function prepareExecution(options) {
  const { expandArgv1, initializeModules, isMainThread, shouldSpawnLoaderWorker, shouldPreloadModules } = options;
  ...
  if (initializeModules) {
    initializeModuleLoaders({ shouldSpawnLoaderWorker, shouldPreloadModules });
  }
...
function initializeModuleLoaders(options) {
  const { shouldSpawnLoaderWorker, shouldPreloadModules } = options;
  ...
  // If shouldSpawnLoaderWorker is true, later when the ESM loader is instantiated on-demand,
  // it will spawn a loader worker thread to handle async custom loader hooks.
  const { initializeESM } = require('internal/modules/esm/utils');
  initializeESM(shouldSpawnLoaderWorker);
