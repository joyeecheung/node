...
function registerHooks(hooks) {
  ...
  if (hooks.resolve) {
    ...
    copy.resolve = hooks.resolve;
  }
  if (hooks.load) {
    validateFunction(hooks.load, 'hooks.load');
    copy.load = hooks.load;
  }
  if (hooks.exports) {
    validateFunction(hooks.exports, 'hooks.exports');
    copy.exports = hooks.exports;
  }
  const id = Symbol('kModuleHooksIdValue');
  copy[hookId] = id;
  ArrayPrototypePush(synchronousHooks, copy);
  return id;
}

function deregisterHooks(id) {
  const index = ArrayPrototypeFindIndex(synchronousHooks, (hook) => hook[hookId] === id);
  if (index !== -1) {
    ArrayPrototypeSplice(synchronousHooks, index, 1);
  }
}
...
function exportsWithHooks(url, format, originalExports) {
  ...
  const exportsContext = new ModuleExportsContext(format);
  ...
  return nextExports[nextExports.length - 1](url, exportsContext);
}
...
function resolveWithHooks(specifier, parentURL, isMain, defaultResolve) {
