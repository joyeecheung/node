...
function loadCJSModule(module, source, url, filename, isMain) {
  ...
  const requireFn = function require(specifier) {
    ...
    const request = { specifier, importAttributes, phase: kEvaluationPhase, __proto__: null };
    const job = cascadedLoader.getModuleJobForRequireInImportedCJS(url, request);
    job.runSync();
