...
// Layout of TickInfo::ImmediateInfo
// [ 4/8 bytes ]  snapshot index of fields
template <>
...
// Layout of PerformanceState::ImmediateInfo
...
void SnapshotData::ToBlob(FILE* out) const {
  ...
  written_total += w.WriteVector<SnapshotIndex>(isolate_data_indices);
...
void SnapshotData::FromBlob(SnapshotData* out, FILE* in) {
  ...
  r.Debug("Read isolate_data_indices\n");
  out->isolate_data_indices = r.ReadVector<SnapshotIndex>();
