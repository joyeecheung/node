...
/**
 *
 * @param {ModuleLoader} cascadedLoader
 * @param {string[]} userImports
 * @param {function(ModuleLoader):ModuleWrap|undefined} callback
 * @return {Promise}
 */
async function handlePreImports(cascadedLoader, userImports, callback) {
  // Finishing all the user --import preloads in this thread.
  const parentURL = getCWDURL().href;
  for (let i = 0; i < userImports.length; i++) {
    await cascadedLoader.import(userImports[i], parentURL, kEmptyObject);
  }
  return callback(cascadedLoader);
}

/**
 * @param {function(ModuleLoader):ModuleWrap|undefined} callback
 * @return {Promise|ModuleWrap|undefined}
 */
function asyncRunEntryPointWithESMLoader(callback) {
  const cascadedLoader = require('internal/modules/esm/loader').getOrInitializeCascadedLoader();
  try {
    const userImports = getOptionValue('--import');
    if (userImports.length === 0) {
      // Block until the async loader hooks have been initialized. No-op
      // if there's none of them.
      cascadedLoader.waitForAsyncLoaderHookInitialization();
      return callback(cascadedLoader);
    }
    return handlePreImports(cascadedLoader, userImports, callback);
  } catch (err) {
