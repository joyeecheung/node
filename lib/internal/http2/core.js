...
class Http2Session extends EventEmitter {
  ...
  destroy(error = NGHTTP2_NO_ERROR, code) {
    ...
    if (!socket.destroyed)
      globalProxy.setImmediate(finishSessionDestroy, this, error);
    else
...
class Http2Stream extends Duplex {
  ...
  sendTrailers(headers) {
    ...
    globalProxy.setImmediate(finishSendTrailers, this, headersList);
  }
  ...
  [kMaybeDestroy](code = NGHTTP2_NO_ERROR) {
    ...
    if (this._writableState.finished) {
      ...
      if (this.headersSent &&
        ...
        globalProxy.setImmediate(this.close.bind(this));
      }
    }
  }
}
...
const setTimeout = {
  ...
  value: function(msecs, callback) {
    ...
    globalProxy.clearTimeout(this[kTimeout]);
