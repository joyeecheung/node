...
const {
  getInternalImportCallback, InternalScript, runScriptInThisContext,
} = require('internal/vm');
...
function evalScript(name, body, breakFirstLine, print, shouldLoadESM = false) {
  ...
  const runScript = () => {
    ...
    const result = module._compile(script, `${name}-wrapper`)(() => {
      ...
      const importModuleDynamically = getInternalImportCallback(baseUrl);
      const script = new InternalScript(
        body,                    // code
        name,                    // filename,
        0,                       // lineOffset
        0,                       // columnOffset,
      ...
      runScriptInThisContext(script, true, !!breakFirstLine);
    });
    if (print) {
