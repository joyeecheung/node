...
static std::unordered_map<uint16_t, std::unique_ptr<PerIsolateWrapperData>>
    wrapper_data_map;
static Mutex wrapper_data_mutex;
...
IsolateData::IsolateData(Isolate* isolate,
...

  {
    ...
    node::Mutex::ScopedLock lock(wrapper_data_mutex);
    auto it = wrapper_data_map.find(cppgc_id);
    if (it == wrapper_data_map.end()) {
      auto pair = wrapper_data_map.emplace(
