...
class ModuleLoader {
   ...
   */
  getModuleJobForImport(parentURL, request) {
    // TODO(joyeecheung): just pass the request around, don't expand them into arguments until they are needed.
    const { specifier, phase, importAttributes } = request;
    const maybePromise = this.resolve(specifier, parentURL, importAttributes);
    ...
    if (isPromise(maybePromise)) {
      return maybePromise.then(afterResolve);
    }
  ...
  async import(specifier, parentURL, importAttributes, phase = kEvaluationPhase, isEntryPoint = false) {
    return onImport.tracePromise(async () => {
      const request = { specifier, phase, importAttributes, __proto__: null };
      const moduleJob = await this.getModuleJobForImport(parentURL, request);
  ...
  resolve(specifier, parentURL, importAttributes) {
    ...
    if (this.isForAsyncLoaderHookWorker) {
      // TODO(joyeecheung): make the synchronous hooks the default step on loader thread.
