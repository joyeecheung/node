...
class ModuleLoader {
  ...
  async getModuleJob(specifier, parentURL, importAttributes) {
    ...
    return this.getJobFromResolveResult(resolveResult, parentURL, importAttributes, this.load.bind(this));
  }
  ...
  getModuleJobForRequireInImportedCJS(specifier, parentURL, importAttributes) {
    ...
    function getSource(url, context) {
      const result = self.loadSync(url, context);
      const format = result.format;
      if (!getOptionValue('--experimental-require-module') && (format === 'module' || format === 'module-typescript')) {
        throw new ERR_REQUIRE_ESM(resolveResult.url, true);
      }
      if (format === 'commonjs') {
        result.format = 'require-commonjs';
      }
      if (format === 'commonjs-typescript') {
        result.format = 'require-commonjs-typescript';
      }
      return result;
    }
    return this.getJobFromResolveResult(resolveResult, parentURL, importAttributes, true, getSource);
  }

  getJobFromResolveResult(resolveResult, parentURL, importAttributes, sync, getSource) {
    ...
    if (job === undefined) {
      job = this.#createModuleJob(url, resolvedImportAttributes, parentURL, format, sync, getSource);
    }
  ...
  getModuleJobForRequire(specifier, parentURL, importAttributes) {
    ...
    // TODO(joyee): use resolveSync here.
   ...
   * @returns {Promise<ModuleJob>} The (possibly pending) module job
   */
  #createModuleJob(url, importAttributes, parentURL, format, sync, getSource) {
    ...
    const moduleProvider = sync ?
      (url, isMain) => callTranslator(getSource(url, context), isMain) :
      async (url, isMain) => callTranslator(await getSource(url, context), isMain);
  ...
  resolveWithMaybeOffThreadHooks(specifier, context) {
    if (this.#customizations) {
      return this.#customizations.resolveSync(specifier, context.parentURL, context.importAttributes);
    }
    return this.cachedDefaultResolve(spcifier, context.parentURL, context.importAttributes);
  }
  ...
  resolve(originalSpecifier, parentURL, importAttributes) {
    if (resolveHooks.length) {  // Has module.registerHooks hooks, use the synchronous variant that can handle both hooks.
      return this.resolveSync(originalSpecifier, parentURL, importAttributes);
    }
    ...
    return this.cachedDefaultResolve(spcifier, parentURL, importAttributes);
  }

  cachedDefaultResolve(specifier, parentURL, importAttributes) {
    // FIXME(joyeecheung): cache should be checked before hooks, at least for module.registerHooks(),
    // or at least configurable for maxium performance.
    const requestKey = this.#resolveCache.serializeKey(specifier, importAttributes);
  ...
  resolveSync(originalSpecifier, parentURL, importAttributes) {
    if (resolveHooks.length) {  // Has module.registerSync hooks, use the synchronous variant that can handle both hooks.
      return resolveWithHooks(originalSpecifier, parentURL, importAttributes, this.resolveWithMaybeOffThreadHooks.bind(this));
    }
    return this.resolveWithMaybeOffThreadHooks(originalSpecifier, { parentURL, importAttributes });
  }
  ...
  loadWithMaybeOffThreadHooks(url, context) {
    if (this.#customizations) {
      return this.#customizations.loadSync(new URL(url), context);
    }
    defaultLoadSync ??= require('internal/modules/esm/load').defaultLoadSync;
    return defaultLoadSync(new URL(url), context);
  }
  ...
  loadSync(url, context) {
    let result;
    if (loadHooks.length) {
      result = loadWithHooks(url.href, context.format, this.loadWithMaybeOffThreadHooks.bind(this));
    } else {
      result = this.loadWithMaybeOffThreadHooks(url, context);
    }
