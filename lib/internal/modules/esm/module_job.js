...
class ModuleJob extends ModuleJobBase {
  ...
  #syncLink(evaluationDepJobs) {
    // Modules should be aligned with the moduleRequests array in order.
    const modules = Array(moduleRequests.length);
    this.#forEachModuleRequest((idx, eidex, request) => {
      const job = this.#loader.getModuleJobForImport(this.url, request);
      debug(`ModuleJob.link() ${this.url} -> ${specifier}`, job);
  ...
  #link() {
    ...
    if (this.#loader.isForAsyncLoaderHookWorker) {
      return this.#asyncLink(evaluationDepJobs);
    }
    return this.#syncLink(evaluationDepJobs);
  }
