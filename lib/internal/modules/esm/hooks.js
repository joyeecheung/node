...
// This shares the same interface as AsyncLoaderHookWorkerProxy, but instances of this class
// run directly on the loader worker thread.
class AsyncLoaderHookOnLoaderWorker {
  #chains = {
    /**
...
ObjectSetPrototypeOf(AsyncLoaderHookOnLoaderWorker.prototype, null);

/**
 * There may be multiple instances of AsyncLoaderHookOnLoaderWorker/AsyncLoaderHookWorkerProxy,
 * but there is only 1 Internal worker, so there is only 1 MessageChannel.
 */
let MessageChannel;
class AsyncLoaderWorker {
  ...
  makeSyncRequest(method, transferList, ...args) {
    ...
    if (response.message.status === 'never-settle') {
      // TODO(joyeecheung): return something that means "never settle" and let
      // the loader handle it.
      const error = new Error('async loader request never settled');
      error.code = 'ERR_ASYNC_LOADER_REQUEST_NEVER_SETTLED';
...
ObjectSetPrototypeOf(AsyncLoaderWorker.prototype, null);
...
exports.AsyncLoaderHookOnLoaderWorker = AsyncLoaderHookOnLoaderWorker;
exports.AsyncLoaderWorker = AsyncLoaderWorker;
